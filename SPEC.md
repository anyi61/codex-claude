...
- [x] Codex 端到端验证：claude_status → claude_query → claude_implement 全链路
- [x] Skill 文件：`.agents/skills/claude-delegate.md`
- [x] 三个独立 JSON Schema（QUERY / REVIEW / IMPLEMENT），不再共用 RESULT_SCHEMA
- [x] maxTurns: query=4, review=6, implement=15
...
| **implement** | 仅当用户显式传 `session_key` 或 `fork_session` 时 resume。不传则新建 session 并记录 session_id，但不自动复用 | `-p -r <session_id>` 或 `-p --fork-session <session_id>` |
...
→ 标记该 session `expired: true`
→ fallback 到无 resume 的新 session 调用
...
- [ ] `npm run build` 通过
- [ ] `debug/mcp-test.ts` 能证明 query 第二次调用使用 `-r`
- [ ] run log 中包含 `session` 字段（requested_session_id / resumed / forked / returned_session_id）
- [ ] `sessions.json` 创建、更新、prune 正常
- [ ] resume 失败时标记 `expired: true` 并自动 fallback
