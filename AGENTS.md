...
- `src/claude-cli.ts`：Claude CLI 的启动、后台作业管理、结果观测与 apply 范围检查。
- `src/jobs.ts`：JobStore 类：基于文件系统的作业持久化、fingerprint 去重、心跳刷新。
- `src/job-runner.ts`：Detached worker 进程入口。
- `src/session.ts`：SessionStore 类：会话持久化与复用策略。
- `src/guard.ts`：cwd 白名单检查、环境清理与递归保护。
- `src/schema.ts`：共享类型、Zod 校验、JSON Schema 以及提示词构建器。
- `src/codex-config.ts`：Codex allow_roots 配置读写。
...
## 测试指南
`npm test` 使用 Vitest 运行 `tests/` 下全部 119 个测试（9 个测试文件）。`npm run build` 和 `npm run typecheck` 也必须在提交前通过。对于行为变更，可同时在 `debug/` 下添加针对性脚本。
