...
- **Package-manager verification scripts：** `npm test`、`npm run <script>`、`yarn test`、`yarn run <script>`、`pnpm test`、`pnpm run <script>` 执行 delegated worktree 中仓库定义的脚本。未配置 `verification.allowedScripts` 时，`npm test` / `yarn test` / `pnpm test` 保持兼容可用；一旦配置 allowlist，这些 test shorthand 会按脚本名 `test` 处理，必须显式列入 allowlist。风险缓解来自：(1) 环境清洗（`sanitizeEnv()` 防止 secret 泄露），(2) 命令解析为 argv 且不经过 shell（防止 shell 注入），(3) `FORBIDDEN_SCRIPT_NAMES` 阻止高风险脚本名（`install`/`publish`/`deploy`/`start` 等）。
...
| 字段 | 类型 | 约束 | 执行效果 |
|------|------|------|----------|
| `verification.allowedScripts` | string[] | 最多 50 项，每项 ≤100 字符，匹配 `[A-Za-z0-9][A-Za-z0-9:_-]*`，禁止 `install`/`deploy`/`publish`/`start`/`serve`/`add`/`remove`/`uninstall`，禁止 shell 元字符，重复自动去重 | **限制性**：约束 `npm run <script>` / `yarn run <script>` / `pnpm run <script>` 的脚本名；配置后也会把 `npm test` / `yarn test` / `pnpm test` 当作脚本名 `test` 校验。不扩展命令范围 |
