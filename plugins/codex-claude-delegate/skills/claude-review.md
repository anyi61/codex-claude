...
For ordinary review requests, prefer `claude_task mode=review`. `claude_task` waits inline by default; if it returns `status="running"`, continue the same job with `claude_task(job_id=...)`. Use `instruction_files` for plan/checklist context. Use direct `claude_review` only for Advanced / Debug workflows or when the user explicitly asks for the bottom-level tool.
...
5. **After delegated write work** — review Claude's diff before any non-preview `claude_apply`
