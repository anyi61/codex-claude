...
ClaudeJobWaitResult             — do_not_start_duplicate_job, age_ms, stale_state (不含短轮询字段)
...
ClaudeTaskResult                — deduped?, do_not_start_duplicate_job?, completed_inline?, waiting?, status?
```
...
| | `claude_task` | 高层入口，按 mode=auto/read/review/write 路由，默认内联等待结果 |
| | `claude_result` | 解析最相关已完成 job/run + next_actions |
| | `claude_apply` | 预览/应用 worktree 变更 |
| | `claude_cleanup` | 清理 delegated worktree |
| **Advanced** | `claude_status/runs/run_inspect/workspace_status/review_gate/job_wait/query/review/implement/jobs/job_result/job_cancel/job_cleanup` | 调试/精细控制/恢复兼容 |

[CHANGED] `claude_job_wait` 已从默认工具移至高级/恢复兼容。默认路径使用 `claude_task` 的内联等待，无需额外轮询。

[CHANGED] `claude_task` 不再暴露 `timeout_sec`。内部 Claude CLI 执行超时固定为 3600 秒。等待超时由 `wait_timeout_sec` 控制（默认 540，最大 540）。
...
## 14. Tool-contract safety rules

- `claude_task` must not expose `max_turns`; explicit turn caps belong only to Advanced / Debug tools.
- `claude_task` must not expose public `timeout_sec`; internal Claude CLI timeout is fixed at 3600 seconds.
- `claude_task` defaults to inline block wait; `wait_timeout_sec` is capped at 540 to stay under the default MCP host timeout of 600 seconds.
...
- `claude_task` next_actions for running jobs must recommend `claude_task(job_id=...)`, not `claude_job_wait`.
- `claude_job_wait` is not in the default enabled tool set. It is an Advanced/Recovery compatibility wrapper using the same inline wait mechanism.
