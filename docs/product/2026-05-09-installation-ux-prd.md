...
`codex-claude-delegate-mcp` 是一个本地 MCP 工具，让 Codex 把读代码、审查、实现等任务委托给 Claude Code 执行。Codex 负责规划、调用、轮询、验收、预览和最终落地；Claude Code 在隔离 git worktree 中执行具体分析或实现。
...
这些工具只能在 Advanced / Debug 文档中出现，或由用户显式启用。
...
```json
{
  ...
  "interaction": {
    ...
    "next_step": "Poll claude_job_wait with this job_id."
  },
  "next_actions": [
    {
...
```json
{
  ...
  "interaction": {
    ...
    "next_step": "Poll claude_job_wait. Do not start a duplicate task."
  }
}
```
  ...
  "next_actions": [
    {
      ...
      "args": {
        "cwd": "/path/to/repo",
        "job_id": "job_xxx",
        "not_before": "2026-05-09T12:00:45.000Z"
      }
    }
  ],
  "interaction": {
    ...
    "state": "waiting",
    "next_step": "Wait until next_allowed_poll_at, then poll this same job_id."
  }
}
```
...
poll_too_soon 必须给 remaining_delay_ms
next_actions 必须始终包含同一个 cwd 和 job_id
stale 状态不能继续建议无限 wait
stale 状态的默认 next_actions 不得引用默认 6 工具之外的工具
```
...
目标：默认 6 工具返回用户可读状态，而不只是 JSON 数据。该能力是首次使用体验的一部分，必须覆盖全部默认 6 工具后才算完成。
...
| `poll_too_soon=true` | 等到 `next_allowed_poll_at`，不要开新任务 |
| `waiting=true` | 继续轮询同一个 `job_id`，不要重复委托 |
...
6. poll job with claude_job_wait
...
5. interaction block 根据用户最终决策提升为 P0，必须覆盖默认 6 工具，包括 `claude_cleanup`。
...
5. `interaction` block 必须覆盖默认 6 工具后才算完成。
